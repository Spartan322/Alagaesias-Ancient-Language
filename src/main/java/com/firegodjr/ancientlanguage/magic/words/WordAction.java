			...
			for (Object obj : selectors) {
				if (obj instanceof EntityLivingBase) {
					...
					((EntityLivingBase) obj)
							.addPotionEffect(new PotionEffect(Potion.resistance.id, 1200, 2, true));
				}
			}
		...
		public void onUse(MagicData energy, Map<String, String> modData, List<?> selectors) {
			...
			for (Object obj : selectors) {
				if (obj instanceof Entity) {
					...
					fatigue += world.getBlock(pos.getIX(), pos.getIY(), pos.getIZ())
							.getBlockHardness(world, pos.getIX(), pos.getIY(), pos.getIZ()) / 4;
					// Breaks blocks with break word
					world.setBlockToAir(pos.getIX(), pos.getIY(), pos.getIZ());
				}
			}
						...
						if (energy.getActualUser() instanceof EntityPlayer) {
							...
							player.addChatComponentMessage(
									new ChatComponentText(EnumChatFormatting.GRAY + "" + EnumChatFormatting.ITALIC
											+ entity.getCommandSenderName() + " is too powerful to kill with magic!"));
						}
					}
				}
			}
		...
		public void onUse(MagicData energy, Map<String, String> modData, List<?> selectors) {
			...
			for (Object obj : selectors) {
				if (obj instanceof EntityLivingBase) {
					...
					BlockPosHit pos = (BlockPosHit) obj;
					// Sets block on fire
					if (world.isAirBlock(pos.getIX(), pos.getIY(), pos.getIZ()))
						world.setBlock(pos.getIX(), pos.getIY(), pos.getIZ(), Blocks.fire);
				}
			}
		...
		public void onUse(MagicData energy, Map<String, String> modData, List<?> selectors) {
			...
			for (Object obj : selectors) {
				if (obj instanceof EntityLivingBase) {
					...
					((EntityLivingBase) obj)
							.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 200, 99, true));
				}
			}
			...
			for (Object obj : selectors) {
				if (obj instanceof BlockPosHit && energy.getActualUser() instanceof Entity) {
					...
					EntityFallingBlock blockEntity = new EntityFallingBlock(world, pos.getIX() + 0.5,
							pos.getIY() + 0.5, pos.getIZ() + 0.5, 
							world.getBlock(pos.getIX(), pos.getIY(), pos.getIZ()));
		...
		public void onUse(MagicData energy, Map<String, String> modData, List<?> selectors) {
			for (Object obj : selectors) {
				if (obj instanceof BlockPosHit && energy.getActualUser() instanceof Entity) {
					...
					if (world.isAirBlock(pos.getIX(), pos.getIY(), pos.getIZ()))
						world.setBlock(pos.getIX(), pos.getIY(), pos.getIZ(), ModBlocks.ghostLight);
				}
			}
